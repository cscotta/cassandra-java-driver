 ...
 *
 * <p>This policy encapsulates another policy: the {@code distance} method is inherited from the
 * child policy; the iterator returned by {@code newQueryPlan} will first return the {@link
 * HostDistance#LOCAL LOCAL} replicas for the query if possible, and fall back to the child policy's
 * query plan otherwise.
 */
public class TokenAwarePolicy implements ChainableLoadBalancingPolicy {
  ...
  public enum ReplicaOrdering {
    ...
    /** Order replicas by token ring topology. */
    TOPOLOGICAL,

    /** Return replicas in a different, random order for each query plan. This is the default. */
    RANDOM,

    /** Return the replicas in the exact same order in which they appear in the child's plan. */
    NEUTRAL
  }
